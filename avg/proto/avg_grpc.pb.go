...
type AvgServiceClient interface {
	Average(ctx context.Context, opts ...grpc.CallOption) (AvgService_AverageClient, error)
}
...
func (c *avgServiceClient) Average(ctx context.Context, opts ...grpc.CallOption) (AvgService_AverageClient, error) {
	stream, err := c.cc.NewStream(ctx, &AvgService_ServiceDesc.Streams[0], "/avg.AvgService/Average", opts...)
	if err != nil {
		return nil, err
	}
	x := &avgServiceAverageClient{stream}
	return x, nil
}

type AvgService_AverageClient interface {
	Send(*AvgRequest) error
	CloseAndRecv() (*AvgResponse, error)
	grpc.ClientStream
}

type avgServiceAverageClient struct {
	grpc.ClientStream
}

func (x *avgServiceAverageClient) Send(m *AvgRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *avgServiceAverageClient) CloseAndRecv() (*AvgResponse, error) {
...
type AvgServiceServer interface {
	Average(AvgService_AverageServer) error
...
func (UnimplementedAvgServiceServer) Average(AvgService_AverageServer) error {
...
func _AvgService_Average_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AvgServiceServer).Average(&avgServiceAverageServer{stream})
}

type AvgService_AverageServer interface {
	SendAndClose(*AvgResponse) error
	Recv() (*AvgRequest, error)
	grpc.ServerStream
}

type avgServiceAverageServer struct {
	grpc.ServerStream
}

func (x *avgServiceAverageServer) SendAndClose(m *AvgResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *avgServiceAverageServer) Recv() (*AvgRequest, error) {
	m := new(AvgRequest)
...
var AvgService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Average",
			Handler:       _AvgService_Average_Handler,
			ClientStreams: true,
		},
	},
	Metadata: "avg.proto",
